                ...
                <div class="ibox">
                    <div class="ibox-head">
                        <div class="ibox-title"> 
                            <h3>Add New Quarter Master Attendance</h3>
                        </div>
                        <div class="col-lg-offset-6">
                            <a href="{{route('puo.print-qmaster-all-att-blank')}}">
                            <button class="btn btn-info btn-md">
                            Print Blank Sheet</button>
                            </a>
                        </div>
                    </div>
                    <form action="{{route('puo.add-qmaster-attendance')}}" method="POST" role="form">
                        @csrf
                        <div class="ibox-body">
                            <div class="row">
                                     ...
                                     <tbody>
                                        @foreach($qmasters as $key=>$singleQmaster)
                                            <tr>
                                              <td>{{$key+1}}</td>
                                                <td>
                                                <input type="hidden" value="{{($singleQmaster->id)}}" name="user_id[]">{{($singleQmaster->cdt_id)}}
                                                </td>
